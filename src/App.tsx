...
const App:React.FC<Props> = (props) => {
          ...
          onEachFeature: (feature:GeoJson.Feature, layer:L.Layer) => {
              ...
              if (feature.geometry.type !== 'Point') {
                  ...
                  layer.on('mouseover', (e) => {
                      ...
                      let content = contains.map((res:any) => {
                          return `<b>${res.properties.getNaam()}</b><br/>
                          <span class="subTextMarker" style="color:${getHexFromColor(res.properties.getColor(), true)};" >${res.properties.getType()} </span>`;
                      }).reverse().join(`<br/>`);
  ...
  const makeMarker = (feature: GeoJson.Feature, latlng:L.LatLng) => {
      ...
      marker.bindPopup(`<div class = "marker">
                  ...
                  <br/>
                  <span class = "subTextMarker" style="color:${getHexFromColor(feature.properties.getColor(), true)};">${feature.properties.getType()}</span><div>
          `, {
          autoPan: false,
          closeButton: false
      });
...
function getHexFromColor(color:string, text?:boolean):string {
      if (color === "turqoise") {
          if (text) {
              return "#15a49f";
          } else {
